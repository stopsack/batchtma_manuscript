---
title: "Batch effects between tissue microarrays"
...
# a) Biomarker list
markers <- readRDS(file = here("data", "preprocessed", "biomarkers_210309.rds"))
...
markersfig2 <- markers %>%
  select(var_use, name, tumors, scoring, int, cells, area) %>%
...
part2 <- markersfig2 %>%
  mutate(Software  = if_else(scoring == "Software", 1, 0),
         Eye       = if_else(scoring == "Eye",      1, 0),
         Intensity = if_else(int == TRUE,           2, 0),
         `% Cells` = if_else(cells == TRUE,         2, 0),
         Area      = if_else(area == TRUE,          2, 0)) %>%
  select(name, Software:Area) %>%
...
part3 <- markersfig2 %>%
  ...
  scale_y_continuous(expand = c(0, 0), breaks = seq(from = 0, to = 1000, by = 500)) +
  labs(x = "", y = "Tumors (n)") +
  coord_flip() +
...
<embed src = "data/fig7/batchtma_fig7_210413.svg" type="image/svg+xml" />
...
Intraclass correlation coefficients (ICCs), quantifying the proportion of variance in biomarker levels attributable to between-tissue microarray differences, across all tumors and after restriction to those 292 tumors on each tissue microarray that have the same clinical/pathological characteristics in terms of Gleason score 3+4, stage pT1/T2, and tissue source being prostatectomy (excluding transurethral resection of the prostate), which is the three-way combination with the largest number of tumors.
...
ICCs after restriction to Gleason score 3+4, pT1/T2, likely no TURPs (largest three-way combination)
